...
* <span style="color:green">**git checkout main^**</span> означает "первый родитель ветки main". 
...
* <span style="color:green">**git merge <название_ветки>**</span> Делаем слияние вашей ветки в ветку, в которой вы находитесь. Команда merge берет все изменения из ветки и добавляет их в ветку master.
...
### **Перемещение ветки (branch forcing)**</span>

Одна из наиболее распространённых целей, для которых используются относительные ссылки - это перемещение веток. Можно напрямую прикрепить ветку к коммиту при помощи опции -f.  <span style="color:green">**git branch -f main HEAD~3**</span> переместит (принудительно) ветку main на три родителя назад от HEAD.
